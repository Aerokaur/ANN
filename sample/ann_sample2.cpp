...
int main(int argc, char **argv)
{
    ...
    cout << "time taken by ANN : " << dur.count() << " \u03BC"
         << "s " << endl;
    ...
    {                            
        ...
        double min_dist = 1e+300;
        for (int idn = 0; idn < nPts; ++idn)
        {
            double dist = 0.0;
            double diff;
            for (int di = 0; di < dim; ++di)
            {
                diff = queryPt[di] - dataPts[idn][di];
                dist += diff * diff;
            }
            dist = sqrt(dist);
            if (dist < min_dist)
        ...
        cout << "nnIdx2 " << nnIdx2 << endl;
    }
    t2 = high_resolution_clock::now();
    dur = duration_cast<microseconds>(t2 - t1);
    cout << "time taken by brute method : " << dur.count() <<  " \u03BC" << "s " << endl;
